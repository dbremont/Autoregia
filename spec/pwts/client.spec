...
> Defines the PWTS web client: the dashboard artifacts rendered from the
> `/pwts/api/analytics` and `/pwts/api/events` endpoints, and the interaction
> model. Built against `spec/ui.spec`.

## Surface

A single dashboard view (no tabs in the skeleton). All artifacts compute live
from the analytics join of PWTS mouse/focus + PKTS keystrokes.
...
[`pwts/analytics.py`](../../pwts/analytics.py).
