# PEOS Automation — Recommended Architecture
...
4. The sources policy file (`config/peos_sources.json`) stays the *desired
   state*; the database holds the *actual state*; a reconcile step moves the
   latter toward the former and never deletes history.

## Phase 2 — embedded feeder (next implementation step)

A background thread inside the PEOS server process. Chosen over a second
...
server.py (startup)
  └─ feeder thread (daemon)
       every PEOS_SWEEP_S (60 s):
         topics ← enabled topics (in-process, no HTTP self-call)
         for topic in due(topic, state_cursor):      # max ~10 per sweep
             poll_one(topic)                          # existing code path
```

- **Due gating**: `(now − last_fetched_ms) ≥ interval_eff`, where
  `interval_eff = (topic.interval_s or source default) × backoff × jitter`.
...
- **Jitter**: ±10% per topic, so cohorts never poll in lockstep.
- **Per-sweep cap** (~10 topics, oldest-fetched first): after downtime or a
  bulk import, recovery is spread over minutes instead of stampeding mirrors.
- **Nitter respect**: stop at the first mirror that answers; `PEOS_NITTER_ALL=1`
  restores the recall-maximizing all-mirrors sweep for rare backfills.
- **Kill switch**: `PEOS_FEEDER=0` disables the thread (local dev against a
...
- **Standalone scheduler** (the `collector.py` pattern, kept): a separate
  ...
  failure mode.
- **Per-source workers + queue**: due topics are pushed onto a bounded queue;
...
- **Observability**: poll outcomes (per sweep: due/fetched/failed/backed-off)
  as documents in a `peos_activity` log and rolled up on the LOOP dashboard;
  source latency and error rates visible per mirror.
- **Reconcile loop**: periodic diff of policy file ↔ DB topics (the
  `sync_sources.py` semantics, automated: create missing, disable
  absent — never delete).
...
| 2026-09 | Search executes inside CouchDB (ddoc `peos-search` + Mango indexes, created idempotently at startup) — `/api/search`. |
