# Agent Operation Organization System
...
> Within the Autoregia Personal Viable System Model (PVSM), AOOS is the
...
Fundamentally, a AOOS exists to maintain persistent, executable representations of
...
By preserving these across time, a AOOS functions as an externalized **operative
...
> AOOS is a single System 1 composite of three cooperating components. The
> conceptual separation between them is real; the deployment boundary is one
> project.

```
AOOS — Agent Operation Organization System  (VSM System 1 – Operations)
 |
 ...
 |     \_ Adapters that normalize external systems into AOOS terms.
 |        Primary: Google Calendar (OAuth 2.0, two-way sync).
 |        Future: task tools, communication sources, other calendars.
 |
 +-- [G] Goal Tracking  — the objective-tracking system
      \_ Measurable objectives (the apex of the action hierarchy) bound to a
         declared target and a measured present, decomposed into Key Results.
         "What future state I am committing to, and how close am I?"
```
...
| **[D] Execution & Actuals** | The actuals layer — "what really happened" | Work sessions (start/stop, actual effort, outcomes), the block↔session deviation | The durable execution *event stream* (that remains PRS-backed; AOOS holds the operational session object) |
...
| **[G] Goal Tracking** | The objective layer — "what I am committing to become/do" | Measurable objectives: targets, Key Results, progress, status, momentum | The *declared end as identity* (that is ASRS/PSMS's job); [G] tracks the operative objective |

> [B] and [C] together realize what the original AOOS formulation called *"Work
> Calendarization"*: [C] brings the platform's calendar in, [B] reasons over it.

## Formulation

> How to think about a `Agent Operation Organization System`?

A `Agent Operation Organization System` is a technical object with the role of
...
> AOOS should organize any action construct whose unorganized, forgotten, or
...
| **Project** | `Project` | A coordinated collection of tasks with a shared outcome. | Personal Recording System; AOOS itself. |
...
> AOOS operational extension. PRS remains the single source of truth for the
> record's content and metadata; AOOS adds only what PRS has no reason to store:
...
#### Added by AOOS (the operational extension)

| Field | Description | Example |
| --- | --- | --- |
| **Action Id** | AOOS-local identifier for the managed action (may alias the record id). | `ACT-2026-00042`. |
...
| **Execution Log** | A stream of execution events. Each event is itself a PRS record (`Event`/`Observation`) linked back to the construct; AOOS does not duplicate it. | `→ REC-2026-00099 (started)`. |

> The execution log is *not* a AOOS-owned store. It is the set of PRS records that
...
> AOOS uses the PRS link vocabulary for general relationships, and adds an
...
> Component [B] is the **calendar system** inside AOOS. It takes the logical action
...
> Component [D] is the **actuals layer** inside AOOS. Where [B] reasons over
> *intended* time (blocks), [D] records *actual* time (work sessions). It is what
> turns AOOS from an open-loop planner into a closed-loop operations system: the
...
> Component [C] is the boundary between AOOS and the external systems the agent
> operates through. Each platform is wrapped by an **adapter** that normalizes the
> platform's objects into AOOS terms (action constructs and calendar blocks) and
...
- **Token storage:** OAuth refresh + access tokens stored in the AOOS local store
...
> Setup steps are documented in the AOOS README; the credentials are brought in
> out-of-band.

#### Sync Model

- **Direction:** **two-way** by default. Constructs created/scheduled in AOOS are
  pushed to Google; events changed in Google are pulled into AOOS.
- **Calendar selection:** the agent selects which Google calendars to sync (e.g.,
  `primary`, `work`, `personal`). Each is mapped to a AOOS `calendar_id`.
- **Change detection:**
...
- **Conflict resolution:** each synced object carries a `sync_state`
  ...
  *local* (agent-authored) change is preferred for AOOS-originated fields; the
...
| Google Calendar (`events`) | Autoregia (AOOS) | Notes |
| --- | --- | --- |
...
| `extendedProperties` (private) | AOOS action id, scheduling state, domain | Carries the Autoregia back-reference so round-trips are unambiguous. |
...
objects, normalize to AOOS terms, push local changes, pull remote changes*.

## Goal Tracking (Component G)

> Component [G] is the **objective-tracking system** inside AOOS. It takes the
> apex of the action hierarchy — the **Objective** — and binds it to a measurable
> **target** (the declared end) and a measured **present** (the current value),
> decomposed into **Key Results**. Where [A] organizes *what to do*, [G] tracks
> *what I am committing to become or achieve, and how close I am*.

> A goal is the **intention** against which AOOS measures **reality** (progress,
> momentum). It is the apex set-point of the operations system: the initiatives,
> projects, and tasks beneath it exist to advance it.

### Principle

> A goal is an Objective action carrying a `goal` extension. The extension binds a
> declared target to a measured present and decomposes the gap into Key Results.
> Progress and momentum are derived; status is assigned from progress against the
> target and the deadline.

Conversely, a plain Objective without a `goal` extension is an untracked objective
— a direction with no measurable commitment. Attaching the extension is what turns
a declared end into a tracked goal.

### Model

The `goal` extension is an optional field on an action construct (present on
Objectives; permitted on Initiatives). See [`schema.json`](schema.json)
§`goalExtension`.

| Field | Description | Example |
| --- | --- | --- |
| **Target** | The declared end: a metric, a value, a unit, and a direction (`gte` higher-is-better / `lte` lower-is-better). | `{metric: "deep-work hours/week", value: 15, unit: "hours", direction: "gte"}`. |
| **Baseline / Current** | The starting value and the latest measured value of the target metric. | baseline `4`, current `9`. |
| **Progress %** | 0–100. The share of the intended change achieved. Computed from Key Results when present, else set manually. | `45`. |
| **Key Results** | The measurable line items that roll up into the goal (OKR-style). Each has its own target/current/direction. | "Mean deep-work hours/week ≥ 15 (now 9)". |
| **Status** | `on-track` · `at-risk` · `off-track` · `achieved` · `dormant`. Assigned from progress and deadline proximity. | `at-risk`. |
| **Horizon** | `quarterly` · `annual` · `long-term`. | `quarterly`. |
| **Deadline** | The date by which the target should be met. | ISO 8601. |
| **Progress History** | A time series of `progress_pct` (`{date, pct}`). Its slope yields **momentum**. | `[{..., pct: 30}, {pct: 42}, {pct: 50}, {pct: 48}]`. |

#### Key Results

A Key Result binds a single measurable quantity to a target. Progress on a KR is
`current ÷ target` (inverted for `lte` directions); a goal's overall progress is
the mean of its KRs (clamped to 0–100) when KRs are present. KR status follows the
same `gte`/`lte` logic, surfacing which line items are carrying the goal and which
are dragging it.

#### Momentum

Momentum is **derived from the progress-history slope**, not stored: a positive
recent slope = `advancing`; flat = `stalled`; negative = `regressing`. It answers
whether the gap to the target is closing or widening — the leading indicator that
distinguishes a goal that is merely *behind* from one that is *falling further
behind*.

### Roll-up

A goal may also roll up from the action hierarchy beneath it: the initiatives,
projects, and tasks whose `strategic.objective` matches the goal. Their completion
provides an alternative ground-truth for progress (mean completion of realized
children). When both KRs and child-roll-up are available, the agent chooses which
governs `progress_pct`; the other is shown as corroboration.

### Evaluation

| Criterion | Indicator | Description |
| --- | --- | --- |
| Goal Advancement | Progress vs Target | Share of the intended change achieved, across goals. |
| Goal Momentum | Advancing/Stalled/Regressing | Count of goals whose gap is closing vs widening. |
| Goal Health | At-Risk / Off-Track | Goals behind pace or past deadline. |
| Goal Completion | Achieved | Goals whose target was met within horizon. |

### Relation to ASRS

The **declared end** of a goal — the objective as an identity/direction facet —
belongs to the Agent Self-Representation System ([PSMS](../../spec/asrs/psms/)
*Commitment* facet, with ranking from [PPS](../../pps/)). Component [G] does not
own the declared end; it **operationalizes** it: a measurable target, tracked
progress, and momentum. The goal in ASRS is the *intention*; the goal in AOOS [G]
is the *measured pursuit* of it.

## Evaluation

> How to evaluate a Agent Operation Organization System?
...
| Calendar Sync Integrity | Sync Drift | Latency and divergence between AOOS and the external calendar |
...
| --- | --- |
| Storage | JSON + SQLite (records live in PRS; AOOS extension tables: dependencies, blocks, external mappings, sync state) |
...
- [PRS — spec](../prs/spec.md) / [schema](../prs/schema.json) — sibling recording system; AOOS operates over its action-bearing records.
