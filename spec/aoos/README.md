# Agent Operation Organization System (AOOS)
...
- **[C] Platforms Integration** — the boundary with the outside world. Adapters
  normalize external systems into AOOS terms. **Google Calendar** is the primary
...
The AOOS prototype serves it at `GET /docs` (see the prototype README), and the
...
3. Download the client secret as `client_secret.json` and place it in the AOOS
   config location (documented in the prototype README; never committed).
4. On first run, AOOS performs the interactive consent flow and stores the refresh
...
- [AOOS — spec](spec.md) — conceptual foundations, data model, calendar & integration architecture.
