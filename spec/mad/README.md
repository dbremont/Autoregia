# Memory Aid System (MAD)

> This document establishes the conceptual foundations, architecture, and implementation of a **Memory Aid System (MAD) (MAD)**. A **MAD** is a technical object engineered to `preserve the continuity and coherence of human agency` by externalizing relevant states and enabling their persistent recording, discovery, retrieval, and use over time.

Fundamentally, a MAD exists to maintain persistent representations of the states that are relevant for effective action and orientation. These include:
...
- By preserving these states across time, a MAD functions as an externalized memory and coordination substrate, reducing state loss, improving continuity, and enabling more reliable individual and collective action.

## Formulation

> How to think about a `Memory Aid System (MAD)`?

A `Memory Aid System (MAD)` is a technical object with the role of externalizing **cognitive and intentional processes** to scaffold extended agency:
...
> A MAD should preserve any state whose disappearance would reduce an agent's ability to reason, decide, coordinate, or act effectively in the future.
...
| ----------------------- | ---------------- | --------------------------------------------- | ---------------------------------------------------- |
| **Intentional State**   | Goals            | Desired future states to achieve.             | Finish MAD implementation; Learn Rust.               |
...
|                         | Priorities       | Relative importance of ongoing work.          | MAD > Website redesign.                              |
...
|                         | Projects         | Collections of coordinated tasks.             | Memory Aid System (MAD).                           |
...
This repository (`tool-pbs`) hosts the **MAD prototype**. The backend is **mocked** (Flask + JSON file); the client is the **actual final version** in terms of UX, visual design, and interaction patterns.
...
The interface follows the spec UI/UX principles: visually calm, consistent, minimal, with aesthetics emerging from clarity, hierarchy, and purposeful design — refined to a classic, editorial register (academic journal × private study × fine print craft).
...
- **Iconography:** Self-hosted Lucide monoline set (1.6px stroke, 24px grid) via a `<mad-icon>` custom element / `MAD.icon()` helper — no Unicode glyphs
