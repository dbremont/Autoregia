...
- [PTOCS](../ptocs/) **catalogs** capabilities — what exists, where it lives, where the gaps are. It is a *map*, not a *program*.
...
ACSMS is a **substrate** system: it sits beside [AGS](/about.html#elements) and PTOCS in the agent's foundation, and is fed by the loop's feedback arc.

```
PRAS (adaptations) ──► ACSMS ──► AIAS  (improvement intents)
       PKTS (signals)    │
       PTOCS (gaps)      └──────► AGS / PTOCS  (capability updates)
...
- **AGS capability facets** — the current tiers of what the agent holds.
- **AGS** — values, strategy, and the learning policy that governs promotion criteria.
...
- **Capability updates → AGS / PTOCS** — new `capability_skill` entries, tier promotions, and retirements, with the evidence that justifies them.
- **Policy amendments → AGS** — changes to how the agent learns (criteria, cadence, culling decisions).
...
- [PTOCS](../ptocs/) — the ledger ACSMS writes into; source of gap analysis and `capability_skill` entries.
- [AGS](/about.html#elements) — the self-model facets (capability, resource, commitment) kept truthful by consolidation.
...
- [AGS](/about.html#elements) — the normative layer; its learning policy defines the tiers and promotion criteria ACSMS enforces.
