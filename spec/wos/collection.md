# WOS Collection Policy

How WOS collects what *other agents* say about the world. This document is
...
[`../wos/README.md`](../../app/module/wos/README.md).
...
1. **Public, no-auth feeds only.** No scraping behind logins, no API keys for
   ...
   session token to serve an individual user's view, WOS does not use it.
...
|---|---|---|
| Poll slowly by default | per-source `default_interval_s` (nitter: 1800 s — "use the RSS, slowly"; arXiv/OpenAlex/Crossref: 6 h; bioRxiv: daily; generic rss: 12 h); spec `interval_s: 0` means "source default" | `sources/*.py`, `poll_one()` |
...
| Identify to polite APIs | OpenAlex and Crossref accept a `mailto` parameter (env `WOS_CONTACT_EMAIL`) that puts us in their polite pool | `sources/openalex.py`, `sources/crossref.py` |
| Never refetch what the DB has | incremental cursors: each source's `STATE-<id>` doc stores `last_fetched_ms`; sources filter by `since_ms`; observations dedup by `sha1(source:native_id)` | `server.py` `poll_one()`, `sources/base.py` `obs_id()` |
| Progress is resumable | cursors live as CouchDB documents, not in memory — restarts never rewind | `STATE-` docs |
| Mirror load is spread | nitter fails over through an ordered instance list, live mirrors first (probed 2026-09-13); dead mirrors are *removed*, not retried forever | `sources/nitter.py`, `config/seed.json` `settings.nitter_instances` |
...
| Be honest about the client | some free feeds (GDELT, Reddit) block non-browser User-Agents; the browser-like UA is required for parity with real readers and is overridable via `WOS_USER_AGENT` | `sources/http_util.py` |
...
[`config/seed.json`](../../app/module/wos/config/seed.json) (the poll set);
the DB holds the cursors and the observations; the seed file holds the
watched sources (see [`policy.md`](policy.md)).
...
- When a source starts failing consistently, prefer *disabling the spec*
  (enabled → false in the sources file, then a sync) over retrying harder.
- Keep default intervals; shorten only with a reason recorded in the spec's
  `note`.
