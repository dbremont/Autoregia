...
> (that is System 1 / PWOS itself, or System 3 Control); it *observes* what was
...
The Todoist shape is a concrete, well-understood projection of PWOS action
constructs — a *platform adapter* in the sense of Component [C]. The metrics,
however, are platform-neutral: they are defined over the PWOS action model and
...
1. **The dataset is synthetic.** A deterministic generator
   (`pwos/data/gen_todoist_mock.py`) produces a plausible, year-long Todoist
...
> adapter contract: how an external platform's objects become PWOS terms.
...
| `name` | Human-readable name. | `"PWOS Prototype"` |
...
| Todoist object | PWOS action construct | Notes |
| --- | --- | --- |
...
> The indicators are organized by the PWOS evaluation families. Each indicator
...
> mean "ship PWOS"; it means *whatever you commit to, you close on time*. They
...
| **Evaluation Mapping** | Table | PWOS Evaluation criteria → the indicator that serves each. |
...
| Layer | Choice |
| --- | --- |
| Dataset | `pwos/data/mock_todoist.json` (generated by `gen_todoist_mock.py`) |
| Computation | Python aggregations in `pwos/server.py` (server-side; the dataset is small) |
...
- [PWOS — spec](spec.md) — parent specification & the Evaluation section this dashboard serves.
- [PWOS — schema](schema.json) — action construct + calendar block data models.
- [PWOS — prototype README](../../pwos/README.md) — how to run the prototype.
