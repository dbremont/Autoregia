"""
Agent Operation Organization System (AOOS) — Prototype Server.

Flask backend implementing the AOOS components:
...
Actions, blocks, and sessions are persisted in CouchDB (db ``aoos``),
...
usable offline. Conforms to spec/aoos/schema.json.

Run:   python3 aoos/server.py
...
except Exception as _exc:  # CouchDB client unavailable -> JSON-fixture fallback
    Store = None
    StoreError = Exception
    print("[aoos] storage/CouchDB client unavailable; using JSON-fixture fallback.")
...
# When CouchDB is unavailable, `store` is None and load()/save() fall back to
# the JSON fixture files directly (read-only-ish demo mode).
store = None
if Store is not None:
    try:
        store = Store("aoos", seed_paths=[ACTIONS_PATH, BLOCKS_PATH, SESSIONS_PATH, SCRATCH_PATH])
    except StoreError as _exc:
        print(f"[aoos] CouchDB connect failed; using JSON-fixture fallback ({_exc}).")
        store = None

# Google credentials (Component C). Brought in out-of-band; never committed.
CLIENT_SECRET_PATH = os.environ.get(
    "AOOS_GC_CLIENT_SECRET",
...
def load(path):
    ...
    Actions/blocks are served from the ``aoos`` CouchDB database (filtered by
...
def save(path, data):
    ...
    """
    if store is not None and path in (ACTIONS_PATH, BLOCKS_PATH, SESSIONS_PATH):
        for doc in data:
...
def update_action(action_id):
    ...
    for k in ("kind", "scheduling_state", "effort_estimate", "capacity_profile",
              "dependencies", "pinned", "strategic", "goal"):
        if k in data:
            a[k] = data[k]
...
def delete_action(action_id):
    if store is not None: store.delete(action_id)
...
# ════════════════════════════════════════════════════════════════════════════
# Component G — Goal Tracking
# ════════════════════════════════════════════════════════════════════════════
def _goal_momentum(history):
    """Derive momentum (advancing/stalled/regressing) from the progress-history slope."""
    pts = sorted((h for h in (history or []) if "pct" in h and "date" in h), key=lambda h: h["date"])
    if len(pts) < 2:
        return "stalled"
    half = max(1, len(pts) // 2)
    first = sum(p["pct"] for p in pts[:half]) / half
    last = sum(p["pct"] for p in pts[half:]) / max(1, len(pts) - half)
    diff = last - first
    if diff > 2:
        return "advancing"
    if diff < -2:
...
def _goal_view(a):
    ...
    history = g.get("progress_history", [])
    return {
        "id": a["id"],
        "record_id": a.get("record_id"),
        "title": target.get("metric") or (a.get("strategic") or {}).get("objective") or a["id"],
        "objective_id": g.get("objective_id") or (a.get("strategic") or {}).get("objective"),
        "scheduling_state": a.get("scheduling_state"),
        "target": target,
        "baseline": g.get("baseline"),
        "current": g.get("current"),
        "progress_pct": g.get("progress_pct", 0),
        "status": g.get("status", "on-track"),
        "horizon": g.get("horizon"),
        "deadline": g.get("deadline"),
        "owner": g.get("owner"),
        "key_results": g.get("key_results", []),
        "progress_history": history,
        "momentum": _goal_momentum(history),
...
def get_goals():
    goals = [gv for a in load(ACTIONS_PATH) if (gv := _goal_view(a))]
...
@app.route("/api/goals/<action_id>", methods=["GET"])
def get_goal(action_id):
    a = next((x for x in load(ACTIONS_PATH) if x["id"] == action_id), None)
    if not a or not a.get("goal"):
        return jsonify({"error": "Goal not found"}), 404
    return jsonify(_goal_view(a))


@app.route("/api/analytics/goals", methods=["GET"])
def analytics_goals():
    """Goal-tracking analytics (Component G): progress, momentum, status, at-risk."""
    goals = [gv for a in load(ACTIONS_PATH) if (gv := _goal_view(a))]
    now = datetime.now(timezone.utc)
    if not goals:
        return jsonify({"count": 0, "avg_progress": 0, "goals": [],
                        "by_status": {}, "by_momentum": {}, "by_horizon": {},
                        "at_risk": [], "achieved": 0})
    by_status, by_momentum, by_horizon = {}, {}, {}
    for g in goals:
        by_status[g["status"]] = by_status.get(g["status"], 0) + 1
        by_momentum[g["momentum"]] = by_momentum.get(g["momentum"], 0) + 1
        if g.get("horizon"):
            by_horizon[g["horizon"]] = by_horizon.get(g["horizon"], 0) + 1
    avg_progress = round(sum(g["progress_pct"] for g in goals) / len(goals))
    at_risk = sorted(
        [g for g in goals if g["status"] in ("at-risk", "off-track")],
        key=lambda g: g["progress_pct"])
    # deadline pressure: days to deadline (None if no deadline)
    for g in goals:
        if g.get("deadline"):
            try:
                dl = parse_ts(g["deadline"])
                g["days_to_deadline"] = (dl - now).days
            except Exception:
                g["days_to_deadline"] = None
        else:
            g["days_to_deadline"] = None
    # progress trajectory: mean progress_pct over time (union of history dates)
    date_pcts = {}
    for g in goals:
        for h in g["progress_history"]:
            date_pcts.setdefault(h["date"], []).append(h["pct"])
    trajectory = [{"date": d, "pct": round(sum(v) / len(v))} for d, v in sorted(date_pcts.items())]
    return jsonify({
        "count": len(goals),
        ...
        "advancing": by_momentum.get("advancing", 0),
        "stalled": by_momentum.get("stalled", 0),
        "regressing": by_momentum.get("regressing", 0),
        "at_risk": [{"id": g["id"], "title": g["title"], "status": g["status"],
                     "progress_pct": g["progress_pct"], "momentum": g["momentum"],
                     "deadline": g.get("deadline"), "days_to_deadline": g.get("days_to_deadline")}
                    for g in at_risk],
...
def delete_block(block_id):
    if store is not None: store.delete(block_id)
...
def delete_session(session_id):
    if store is not None: store.delete(session_id)
...
def _get_scratch_doc():
    ...
    if isinstance(raw, dict):
        ...
        doc = next((d for d in raw if isinstance(d, dict) and d.get("id") == SCRATCH_DOC_ID), None)
    if not doc:
        now = now_iso()
        doc = {"id": SCRATCH_DOC_ID, "body": "", "shares": [], "created_at": now, "updated_at": now}
    doc.setdefault("shares", [])
    return doc
...
def scratch_share_page(token):
    """Public share page (served to humans at /aoos/share/<token>)."""
...
# AOOS_GC_CLIENT_SECRET env var), the adapter reports status "mock" and the
...
# AOOS is a single-user local prototype, so an in-memory dict keyed by the
...
def _gc_build_service():
    ...
    try:
        ...
        print(f"[AOOS] Google Calendar service unavailable: {exc}")
        return None
    ...
    if not os.path.exists(CLIENT_SECRET_PATH):
        return jsonify({"status": "mock",
                        "detail": ("No client_secret.json found at %s. "
                                   "See spec/aoos/README.md Google Calendar setup."
                                   % CLIENT_SECRET_PATH)}), 400
    try:
        from google_auth_oauthlib.flow import Flow
    except ImportError:
        return jsonify({"status": "error",
                        "detail": "google-auth-oauthlib not installed; run: pip install -r aoos/requirements.txt"}), 500
...
# Conforms to spec/aoos/analytics.md. Never mutates actions or blocks.
# ============================================================================
def _load_todoist():
...
def analytics_export():
    ...
    return Response(json.dumps(payload, indent=2, default=str),
                    ...
                    headers={"Content-Disposition": "attachment;filename=aoos_analytics.json"})
...
def export_data():
    ...
    return Response(json.dumps(payload, indent=2, default=str),
                    mimetype="application/json",
                    headers={"Content-Disposition": "attachment;filename=aoos_export.json"})
