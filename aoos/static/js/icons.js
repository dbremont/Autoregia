/* ============================================================
   ...
   Use:  AO.icon("gauge")            -> full <svg> markup
         <ao-icon name="gauge"></ao-icon>
...
;(function (AO) {
  ...
  AO.icon = function(name, size) {
    var inner = AO._icons[name] || AO._icons["circle"];
    var s = size || 18;
    return "<svg class=\"ao-icon\" width=\"" + s + "\" height=\"" + s + "\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.6\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">" + inner + "</svg>";
  };

  if (!customElements.get("ao-icon")) {
    customElements.define("ao-icon", class extends HTMLElement {
      connectedCallback() {
        var name = this.getAttribute("name") || "circle";
        var size = parseInt(this.getAttribute("size") || "18", 10);
        this.innerHTML = AO.icon(name, size);
      }
    });
  }
})(window.AO = window.AO || {});
