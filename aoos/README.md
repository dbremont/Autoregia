# Agent Operation Organization System (AOOS) — Prototype
...
> [`../spec/aoos/`](../spec/aoos) ([`spec.md`](../spec/aoos/spec.md),
> [`schema.json`](../spec/aoos/schema.json)).
...
- **[G] Goal Tracking** — measurable objectives (the apex of the action
  hierarchy) bound to a declared target and a measured present, decomposed into
  Key Results, with status, derived momentum, and a **check-in** to update
  reality (`/api/goals`, `/api/analytics/goals`, the Goals view, and goal
  analytics in the Dashboard + Analytics).
- **Analytics Dashboard** — an S3\* (Audit) read-only surface over a synthetic
  ...
  auto-insights. Specified in [`../spec/aoos/analytics.md`](../spec/aoos/analytics.md).
...
PPS → 5004). Override with the `AOOS_PORT` environment variable.
...
generator validates every entry against `spec/aoos/schema.json`.
...
3. Download the client secret as `config/client_secret.json` inside this folder
   (or set `AOOS_GC_CLIENT_SECRET` to its absolute path). **Never commit it.**
...
└── static/
    ...
    └── js/
        ...
        ├── icons.js       # self-hosted Lucide icon set (<ao-icon>)
...
- **Storage substrate:** [PRS](../prs/) — AOOS operates over PRS records; it is
...
- [AOOS — spec](../spec/aoos/spec.md) · [schema](../spec/aoos/schema.json) · [analytics](../spec/aoos/analytics.md)
