/* ============================================================
   ...
   Use:  PW.icon("gauge")            -> full <svg> markup
...
;(function (PW) {
  ...
  PW.icon = function(name, size) {
    var inner = PW._icons[name] || PW._icons["circle"];
    var s = size || 18;
  ...
  if (!customElements.get("pw-icon")) {
    customElements.define("pw-icon", class extends HTMLElement {
      connectedCallback() {
        var name = this.getAttribute("name") || "circle";
        var size = parseInt(this.getAttribute("size") || "18", 10);
        this.innerHTML = PW.icon(name, size);
      }
    });
  }
})(window.PW = window.PW || {});
