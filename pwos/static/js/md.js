/* PWOS Markdown + LaTeX renderer (Component S shared surface).
 *
...
window.PW = window.PW || {};
PW.Markdown = PW.Markdown || {};

PW.Markdown._esc = function (s) {
  return String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;').replace(/'/g, '&#39;');
};

PW.Markdown._renderMath = function (tex, display) {
  if (window.katex) {
    try {
      return window.katex.renderToString(tex, { displayMode: !!display, throwOnError: false });
    } catch (e) { /* fall through */ }
  }
  return '<span class="md-math-fallback">' + PW.Markdown._esc(tex) + '</span>';
};
...
PW.Markdown._extractMath = function (text, stash) {
  let t = String(text);
  t = t.replace(/\$\$([\s\S]+?)\$\$/g, function (m, tex) { return stash(PW.Markdown._renderMath(tex, true)); });
  t = t.replace(/\\\[([\s\S]+?)\\\]/g, function (m, tex) { return stash(PW.Markdown._renderMath(tex, true)); });
  t = t.replace(/\\\(([\s\S]+?)\\\)/g, function (m, tex) { return stash(PW.Markdown._renderMath(tex, false)); });
  // inline $...$: opener not preceded by $, first/last inner char not space or $, single line.
  t = t.replace(/\$([^\s$])([^$\n]*?[^\s$])?\$/g, function (m, a, b) {
    return stash(PW.Markdown._renderMath(a + (b || ''), false));
  });
  return t;
};

PW.Markdown._inline = function (s) {
...
PW.Markdown.render = function (body) {
  ...
  const raw = PW.Markdown._extractMath(body, stash);

  const lines = raw.replace(/\r\n/g, '\n').split('\n');
  const out = [];
  let para = [];
  function flush() { if (para.length) { out.push('<p>' + PW.Markdown._inline(para.join(' ')) + '</p>'); para = []; } }
  let i = 0;
  while (i < lines.length) {
    ...
    if (/^```/.test(line)) {                                              // fenced code
      ...
      i++;
      out.push('<pre><code>' + PW.Markdown._esc(buf.join('\n')) + '</code></pre>');
      continue;
    }
    if (/^\s*$/.test(line)) { flush(); i++; continue; }
    const hm = line.match(/^(#{1,6})\s+(.*)$/);                           // heading
    if (hm) { flush(); out.push('<h' + hm[1].length + '>' + PW.Markdown._inline(PW.Markdown._esc(hm[2])) + '</h' + hm[1].length + '>'); i++; continue; }
    ...
    if (tm) {
      ...
      out.push('<div class="md-task' + (done ? ' is-done' : '') + '"><input type="checkbox" disabled' +
        (done ? ' checked' : '') + '> ' + PW.Markdown._inline(PW.Markdown._esc(tm[2])) + '</div>');
      i++; continue;
    }
    if (/^\s*[-*]\s+/.test(line)) {                                       // unordered list
      flush(); const items = [];
      while (i < lines.length && /^\s*[-*]\s+/.test(lines[i])) { items.push('<li>' + PW.Markdown._inline(PW.Markdown._esc(lines[i].replace(/^\s*[-*]\s+/, ''))) + '</li>'); i++; }
      out.push('<ul>' + items.join('') + '</ul>'); continue;
    }
    if (/^\s*\d+\.\s+/.test(line)) {                                      // ordered list
      flush(); const items = [];
      while (i < lines.length && /^\s*\d+\.\s+/.test(lines[i])) { items.push('<li>' + PW.Markdown._inline(PW.Markdown._esc(lines[i].replace(/^\s*\d+\.\s+/, ''))) + '</li>'); i++; }
      out.push('<ol>' + items.join('') + '</ol>'); continue;
    }
    if (/^\s*>\s?/.test(line)) {                                          // blockquote
      flush(); const buf = [];
      while (i < lines.length && /^\s*>\s?/.test(lines[i])) { buf.push(PW.Markdown._esc(lines[i].replace(/^\s*>\s?/, ''))); i++; }
      out.push('<blockquote><p>' + PW.Markdown._inline(buf.join('<br>')) + '</p></blockquote>'); continue;
    }
    para.push(PW.Markdown._esc(line)); i++;                               // paragraph
  }
  flush();
