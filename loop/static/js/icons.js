/* ============================================================
   LOOP Icons - Self-hosted Lucide icon set (ISC license)
   Curated monoline icons, 1.6px stroke, 24px grid.
   Use:  LOOP.icon("gauge")           -> full <svg> markup
         <loop-icon name="gauge"></loop-icon>
   ============================================================ */

;(function (LOOP) {
  LOOP._icons = {};
  // --- icons part 1 ---
  Object.assign(LOOP._icons, {
  ...
  // --- icons part 3 (loop dashboard extensions) ---
  Object.assign(LOOP._icons, {
  ...
  LOOP.icon = function(name, size) {
    var inner = LOOP._icons[name] || LOOP._icons["circle"];
    var s = size || 18;
    return "<svg class=\"loop-icon\" width=\"" + s + "\" height=\"" + s + "\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.6\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">" + inner + "</svg>";
  };

  if (!customElements.get("loop-icon")) {
    customElements.define("loop-icon", class extends HTMLElement {
      connectedCallback() {
        var name = this.getAttribute("name") || "circle";
        var size = parseInt(this.getAttribute("size") || "18", 10);
        this.innerHTML = LOOP.icon(name, size);
      }
    });
  }
