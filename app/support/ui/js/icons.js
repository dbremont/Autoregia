/* ════════════════════════════════════════════════════════════
   Autoregia Shared Icon Registry (served at /ui/js/icons.js)
   Merged Lucide set (ISC) across all tools — 1.6px stroke, 24px grid.
   Tools register their element via AUTOREGIA.defineIconElement and
   keep a thin <TOOL>.icon wrapper; per-tool copies are gone.
   ════════════════════════════════════════════════════════════ */
...
AUTOREGIA.icon = function (name, size, cls) {
  const inner = AUTOREGIA.ICONS[name] || AUTOREGIA.ICONS["circle"];
  const s = size || 18;
  return '<svg class="' + (cls || "autoregia-icon") + '" width="' + s + '" height="' + s + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' + inner + '</svg>';
};

AUTOREGIA.defineIconElement = function (elementName, cls) {
  if (customElements.get(elementName)) return;
  customElements.define(elementName, class extends HTMLElement {
    connectedCallback() {
      this.innerHTML = AUTOREGIA.icon(this.getAttribute("name") || "", this.getAttribute("size") || 18, cls);
    }
  });
};
