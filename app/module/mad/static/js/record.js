/* ════════════════════════════════════════════════════════════
   MAD Record — CRUD, Editor, Viewer, List
   ════════════════════════════════════════════════════════════ */
MAD.record = {
  currentFilter: null,
...
MAD.record.renderList = function() {
  const records = this.currentFilter
    ? MAD.Store.search(this.currentFilter)
    : this.currentTypeFilter
      ? MAD.Store.getByType(this.currentTypeFilter)
      : MAD.Store.getAll();
  ...
  return `
    <div class="content-header">
      ...
      <div class="actions">
        <button class="btn btn-secondary btn-sm" onclick="MAD.record.openEditor()"><mad-icon name="plus" size="15"></mad-icon> New Record</button>
      </div>
    </div>
    <div class="list-toolbar">
      <select onchange="MAD.record.filterByStatus(this.value)">
      ...
      <select onchange="MAD.record.filterByPriority(this.value)">
    ...
    <div class="record-list">
      ${records.length === 0 ? '<div class="empty-state"><div class="empty-icon"><mad-icon name="file-text" size="46"></mad-icon></div><h3>No records found</h3><p>Create your first record to get started.</p></div>'
        : records.map((r,i) => this.cardHTML(r,i)).join('')}
    </div>`;
};

MAD.record.cardHTML = function(r, i) {
  ...
  return `
    <div class="record-card animate-in delay-${(i%5)+1}" style="--rc-accent:${typeColor};" onclick="MAD.record.showDetail('${r.id}')">
      ...
      <div class="rc-meta">
        ...
        ${r.domain ? `<span class="rc-domain"><mad-icon name="folder" size="13"></mad-icon> ${r.domain}</span>` : ''}
...
MAD.record.esc = function(s) {
  if (!s) return '';
  const d = document.createElement('div'); d.textContent = s; return d.innerHTML;
};

MAD.record.filterByType = function(type) {
  this.currentTypeFilter = type;
  MAD.navigate('records');
};

MAD.record.filterByStatus = function(status) {
  // Re-render with status filter applied
  const container = document.getElementById('appContent');
  let records = this.currentTypeFilter ? MAD.Store.getByType(this.currentTypeFilter) : MAD.Store.getAll();
...
MAD.record.filterByPriority = function(pri) {
  const container = document.getElementById('appContent');
  let records = this.currentTypeFilter ? MAD.Store.getByType(this.currentTypeFilter) : MAD.Store.getAll();
...
// ── Record Editor Modal ────────────────────────────────
MAD.record.openEditor = function(id) {
  const rec = id ? MAD.Store.getById(id) : null;
...
MAD.record.closeModal = function() {
  document.getElementById('recordModal').classList.add('hidden');
  if (this._modal) { this._modal.close(); this._modal = null; }
};

MAD.record.editorForm = function(rec) {
  return `<div class="form-group"><label for="edContent">Content *</label>
    ...
    <div class="meta-grid">
      ...
      <div class="form-group"><label for="edProject">Project</label><input type="text" id="edProject" value="${rec?this.esc(rec.project||''):'MAD Implementation'}"></div>
    ...
    <div class="meta-section open">
      <div class="meta-section-header" onclick="this.parentElement.classList.toggle('open')">
        <span class="chevron"><mad-icon name="chevron-right" size="14"></mad-icon></span> Tags
      </div>
      <div class="meta-section-body">
        <div class="tag-input-container" id="tagInputContainer" onclick="this.querySelector('input').focus()">
          ...
          <input type="text" placeholder="Add tag + Enter" aria-label="Add tag" onkeydown="if(event.key==='Enter'){event.preventDefault();MAD.record.addTagFromInput(this);}">
        </div>
      </div>
    </div>`;
};

MAD.record.tagChipHTML = function(t) {
  return `<span class="tag-chip">${this.esc(t)}<button type="button" class="remove-tag" aria-label="Remove tag ${this.esc(t)}" onclick="event.stopPropagation();this.parentElement.remove()"><mad-icon name="x" size="11"></mad-icon></button></span>`;
};

MAD.record.addTagFromInput = function(input) {
...
document.addEventListener('click', (e) => {
  if (e.target.id === 'btnSaveRecord') { MAD.record.saveCurrent(); }
});

MAD.record.saveCurrent = async function() {
  const getTags = () => {
  ...
  const rec = await MAD.Store.add(data);
  this.closeModal();
  MAD.navigate(MAD.currentView);
  MAD.toast(rec && rec._persisted === false ? 'Record saved locally — sync pending' : 'Record saved');
};
...
// ── Detail Viewer ──────────────────────────────────────
MAD.record.showDetail = function(id) {
  const rec = MAD.Store.getById(id);
  if (!rec) return;
...
MAD.record.detailHTML = function(r) {
  ...
  return `<div class="record-detail">
    <div class="detail-header">
      ...
      <button class="btn btn-secondary btn-sm" onclick="MAD.record.closeDetail();MAD.record.openEditor('${r.id}')">Edit</button>
    </div>
    <div class="detail-meta-bar">
      <span class="badge badge-${sc}">${r.status}</span>
      <span class="badge pri-${pc}">${r.priority}</span>
      ${r.domain?`<span class="rc-domain"><mad-icon name="folder" size="14"></mad-icon> ${this.esc(r.domain)}</span>`:''}
    ...
    ${r.links?.length?`<div class="detail-section"><h4>Relationships</h4><ul class="link-list">${r.links.map(l=>`<li class="link-item" onclick="MAD.record.showDetail('${l.target}')"><span class="link-type">${l.type}</span> <mad-icon name="arrow-right" size="13"></mad-icon> ${l.target}</li>`).join('')}</ul></div>`:''}
    ...
    <div class="meta-section">
      <div class="meta-section-header" onclick="this.parentElement.classList.toggle('open')">
        <span class="chevron"><mad-icon name="chevron-right" size="14"></mad-icon></span> All Metadata
      </div>
      <div class="meta-section-body">
