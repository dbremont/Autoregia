...
ACSMS.Documentation = {
  render() {
    ...
    return `<div class="animate-in">${ACSMS.view.header('Documentation')}<div class="docs-page"><div class="docs-prose">
      ...
      <p>A <strong>skill</strong> is a capability definition: name, description, tags, a <strong>domain</strong> (free-form grouping — Data &amp; Analytics, Language, Reasoning…), a self-assessed <strong>level</strong> 0–5 shown as the catalog progress bar, a lifecycle status (active, paused, retired) and a cadence target (practices per week). A <strong>practice</strong> is one self-reported session bound to an <em>existing</em> skill — you cannot report practice on a skill that does not exist. Each report carries a date, duration, notes, a quality and confidence self-rating (1–5), and optionally a link to the evidence output.</p>
      <h4>Skill paths</h4>
      <p>A <strong>path</strong> sequences skills into an ordered curriculum. Progress is derived, never asserted: the completion bar is the share of members with any practice, and the <em>current step</em> is the first member still needing work — log practice on it straight from the stepper.</p>
