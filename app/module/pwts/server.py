"""
Personal Workstation Tracking System (PWTS) — API Server.

Flask backend serving mouse + focus telemetry. Raw events are accepted at
``/api/ingest`` (written to CouchDB db ``pwts_raw``), asynchronously processed
by an RQ worker (:mod:`pwts.jobs`) into the schema defined in
``spec/pwts/schema.json``, and served back to the web client from CouchDB db
``pwts``. Telemetry is captured by :mod:`pwts.collector`.

This app is mounted under the ``/pwts/`` prefix by the unified dispatcher
(``app.py``); run via the dispatcher so the ``/pwts/`` routes resolve.

Run:   python3 app.py                 (unified dispatcher, port 8080)
       python3 pwts/worker.py         (RQ worker — second terminal)
       python3 pwts/collector.py      (interaction collector — third terminal)
Open:  http://localhost:8080/pwts/
"""
import json
import os
import sys
...
from pwts.jobs import process_pending
from pwts.tasks import enqueue
from pwts.analytics import compute_analytics
...
def ingest():
    ...
    the request path on the ``pwts`` RQ queue. Redis outage never loses data —
...
def analytics():
    """App-interaction analytics — the join of PWTS mouse/focus + PKTS keystrokes.
...
def export_data():
    ...
    return Response(json.dumps(payload, indent=1), mimetype="application/json",
                    headers={"Content-Disposition": "attachment;filename=pwts_export.json"})
