"""Asynchronous interaction processing for PWTS.

Raw batches (posted to ``/api/ingest``) live in CouchDB db ``pwts_raw`` as
documents of shape::

    {id, doc_type:"batch", received_at, processed:bool,
     source_info:{}, device_info:{}, events:[RawEvent]}

where each ``RawEvent`` is emitted by :mod:`pwts.collector` and is either a
...
schema envelope defined in ``spec/pwts/schema.json``, and writes the result
into CouchDB db ``pwts`` — the database the read endpoints serve to the web
client and the analytics layer.

Cross-batch continuity (open session, monotonic event counter, in-flight
click presses) is persisted in a single cursor document in ``pwts_raw`` so
...
def _stable_event_id(seq):
    digest = hashlib.sha256(f"pwts-{seq}".encode()).hexdigest()[:10].upper()
...
def _source_block(batch):
    ...
    return {
        ...
        "collector": info.get("collector", "pwts-collector"),
...
def process_pending():
    ...
    """
    raw_store = Store("pwts_raw")
    proc_store = Store("pwts")
