#!/usr/bin/env python3
"""PWTS interaction collector daemon.

Captures three signals PKTS does not, and streams them to the PWTS ingest
endpoint:
...
1. **Mouse events** — button presses/releases (``click_down``/``click_up``),
   ...
   accumulated and emitted at most every ``PWTS_MOVE_SAMPLE_INTERVAL_MS``
...
Override auto-selection with ``PWTS_BACKEND=evdev|pynput|auto``.
...
Run:  python3 pwts/collector.py
Env:  PWTS_BACKEND                   (default auto)
      PWTS_INGEST_URL                (default http://localhost:8080/pwts/api/ingest)
      PWTS_FLUSH_INTERVAL_S          (default 5)
      PWTS_FLUSH_BATCH_SIZE          (default 50)
      PWTS_MOVE_SAMPLE_INTERVAL_MS   (default 200)
      PWTS_FOCUS_POLL_INTERVAL_S     (default 0.5)
      PWTS_SCREEN_W / PWTS_SCREEN_H  (default 1920 / 1080; used to clamp integrated coords)
"""
import json
import os
import socket
...
from support.shared import focus_watcher  # noqa: E402
...
INGEST_URL = os.environ.get("PWTS_INGEST_URL", "http://localhost:8080/pwts/api/ingest")
FLUSH_INTERVAL_S = float(os.environ.get("PWTS_FLUSH_INTERVAL_S", "5"))
FLUSH_BATCH_SIZE = int(os.environ.get("PWTS_FLUSH_BATCH_SIZE", "50"))
MOVE_SAMPLE_INTERVAL_MS = int(os.environ.get("PWTS_MOVE_SAMPLE_INTERVAL_MS", "200"))
FOCUS_POLL_INTERVAL_S = float(os.environ.get("PWTS_FOCUS_POLL_INTERVAL_S", "0.5"))
SCREEN_W = int(os.environ.get("PWTS_SCREEN_W", "1920"))
SCREEN_H = int(os.environ.get("PWTS_SCREEN_H", "1080"))
MAX_BUFFER = 20000
...
# ═══════════════════════════════════════════════════════════════════════
def select_backend():
    desired = os.environ.get("PWTS_BACKEND", "auto").lower()
    if desired == "evdev":
        pointers = evdev_find_pointers()
        if not pointers:
            raise RuntimeError("PWTS_BACKEND=evdev but no readable pointer devices")
        return "evdev", pointers
    if desired == "pynput":
        if not _PYNPUT_AVAILABLE:
            raise RuntimeError("PWTS_BACKEND=pynput but pynput is not installed")
        return "pynput", None
    pointers = evdev_find_pointers()
    if pointers:
        return "evdev", pointers
