...
This module is the analytical core of PWTS. It reads:

- PWTS processed events (CouchDB db ``pwts``) — mouse interactions and focus
  segments, owned by PWTS.
- PKTS processed events (CouchDB db ``pkts``) — keystrokes, owned by PKTS,
  read **only** here (PWTS never writes to it).
...
PWTS ``focus.app`` / ``mouse.focus.app`` against PKTS
