"""Redis-backed RQ queue for PWTS asynchronous processing.

The ingestion endpoint writes raw interaction batches to CouchDB (db
``pwts_raw``) and enqueues :func:`pwts.jobs.process_pending` on the ``pwts``
queue. A worker process (``python3 pwts/worker.py``) consumes the queue and
