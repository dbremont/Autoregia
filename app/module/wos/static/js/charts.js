/* ════════════════════════════════════════════════════════════
   WOS Charts — Apache ECharts wrapper
...
WOS.Charts = (() => {
  ...
  // on-click bus: every chart wires WOS.Charts.onClick where relevant
  const clickHandlers = {};
  ...
  function treemap(id, items, opts={}) {
    ...
    ch.setOption(base({
      ...
      series:[{ type:'treemap', roam:false, nodeClick:false,
        ...
        label:{ show:true, color:css('--paper-on-accent')||'#FAF1E6', fontFamily:'Inter', fontSize:11, formatter:(p)=> p.value>2?p.name:'' },
