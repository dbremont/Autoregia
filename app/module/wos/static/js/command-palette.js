...
WOS.CommandPalette = {
  ...
  renderResults(query){
    ...
    const navCmds=WOS.VIEWS.map(t=>({icon:t.icon,title:t.label,sub:`Go to ${t.label} — ${t.desc}`,action:()=>{this.close();WOS.navigate(t.id);}}));
    const commands=[
      ...
      {icon:'clipboard-list',title:'Documentation',sub:'About this dashboard',action:()=>{this.close();WOS.openDocs();}},
