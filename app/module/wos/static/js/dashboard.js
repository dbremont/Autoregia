/* ════════════════════════════════════════════════════════════
   WOS Dashboard — "The World Right Now".
   Live view over all sources: KPI cards with sparklines and a
   relevance gauge, volume+relevance over 7 days, top sources with
...
WOS.Dashboard = (() => {
  ...
  const rel = o => o.score != null ? Math.max(0, Math.min(1, o.score / 100))
                                   : Math.round((WOS.Store.toneOf(o) + 1) / 2 * 100) / 100;
  ...
  const winObs = list => qf === 'unread' ? list.filter(o => !WOS.Store.isRead(o.id))
  ...
  function kpis(a, obs, vol) {
    ...
    const t = vol.total || [];
    return `<div class="kpi-card"><div class="kpi-ic">${WOS.icon('table', 16)}</div><div class="kpi-t">Total Signals</div><div class="kpi-v">${a.n || 0}</div><div class="kpi-d ${pctCls}">${pctTxt}</div><div class="kpi-sp">${areaspark(t.slice(-14), 120, 26, '#22a06b')}</div></div>
      <div class="kpi-card"><div class="kpi-ic">${WOS.icon('zap', 16)}</div><div class="kpi-t">Active Sources</div><div class="kpi-v">${nSrc}</div><div class="kpi-d">${nSrc ? '— stable' : '— none'}</div><div class="kpi-sp">${areaspark(Object.values(a.sources || {}).slice(0, 14), 120, 26, '#3b82f6')}</div></div>
      <div class="kpi-card"><div class="kpi-ic">${WOS.icon('calendar', 16)}</div><div class="kpi-t">Latest Bucket</div><div class="kpi-v" style="font-size:16px">${esc(latest)}</div><div class="kpi-d">— current</div><div class="kpi-sp">${areaspark(t.slice(-14), 120, 26, '#7c5cff')}</div></div>
  ...
  function chart7(obs, vol) {
    ...
    const all = WOS.Store.observations();
  ...
  function render() {
    ...
    const obs = winObs(WOS.Store.observations());
    const vol = a.volume || { buckets: [], total: [] };
    const now = new Date();
    const dateStr = now.toISOString().slice(0, 10) + ' · ' + now.toTimeString().slice(0, 5);

    const unread = WOS.Store.observations().filter(o => !WOS.Store.isRead(o.id)).length;
    const today = WOS.Store.observations().filter(o => dayKey(o.observed_at_ms) === dayKey(Date.now())).length;
    const hi = WOS.Store.observations().filter(o => rel(o) >= 0.6).length;
    const pill = (id, label, n, on) => `<button class="qf-pill ${qf === id ? 'on' : ''}" data-qf="${id}">${label}${n != null ? ` (${n})` : ''}</button>`;

    const srcs = {};
    obs.forEach(o => { srcs[o.source || '?'] = (srcs[o.source || '?'] || 0) + 1; });
    const top = Object.entries(srcs).sort((x, y) => y[1] - x[1]).slice(0, 6);
    ...
    const topicRows = top.map(([t, c], i) => {
      ...
      return `<div class="topic-row"><span class="topic-ic" style="background:${col}18;color:${col}">${WOS.icon(ic, 14)}</span><span class="topic-nm">${esc(t)}</span><span class="topic-n">${c} <span class="up">↑</span></span><span class="topic-bar"><i style="width:${(c / maxT) * 100}%;background:${col}"></i></span></div>`;
    }).join('') || '<div class="empty-inline">nothing yet — wait for the next poll</div>';
    ...
    return `
      ...
      <div class="dash-grid">
        ...
        <div class="dash-cols">
          <div class="panel"><div class="phead"><span class="t">Top Sources</span><span class="r">feeds with highest signal volume</span></div>
        ...
        <aside class="dash-rail">
          ...
          <div class="panel"><div class="phead"><span class="t">Quick Filters</span></div>
            <div class="pbody qf">${pill('all', 'All', null, true)}${pill('unread', 'Unread', unread)}${pill('today', 'Today', today)}${pill('week', 'This Week', WOS.Store.observations().length)}${pill('rel', 'High Relevance', hi)}</div></div>
          <div class="panel"><div class="phead"><span class="t">Your Focus</span></div>
            <div class="pbody focus"><span class="focus-ic">${WOS.icon('target', 15)}</span><span>AI, Science, Technology</span><span class="focus-go" onclick="location.hash='#sources'">→</span></div></div>
        </aside>
      </div>`;
  }

  async function run(patch) {
    WOS.navigate('dashboard');
    if (patch) await WOS.Store.applyFilter(patch);
    WOS.navigate('dashboard');
  }
  ...
  function afterRender() {
    ...
    if (g && WOS.Charts.gauge) {
      const obs = WOS.Store.observations(), rels = obs.map(rel);
      const mean = rels.length ? rels.reduce((s, x) => s + x, 0) / rels.length : 0;
      WOS.Charts.gauge('gaugeRel', Math.round(mean * 100), { min: 0, max: 100, color: 'var(--oxford)', fmt: x => (x / 100).toFixed(2) });
    }
    document.querySelectorAll('#chartMode button').forEach(b => b.onclick = () => { chartMode = b.dataset.m; WOS.navigate('dashboard'); });
    document.querySelectorAll('.qf-pill').forEach(b => b.onclick = async () => {
      const id = b.dataset.qf; qf = id;
      if (id === 'today') { WOS.Store.setWindow(24); await WOS.Store.loadAnalytics(); await WOS.Store.loadObservations(); }
      if (id === 'week') { WOS.Store.setWindow(168); await WOS.Store.loadAnalytics(); await WOS.Store.loadObservations(); }
      if (id === 'all') { WOS.Store.setWindow(168); await WOS.Store.loadAnalytics(); await WOS.Store.loadObservations(); }
      WOS.navigate('dashboard');
    });
    // deeper source counts: pull a larger sample straight from /api/search
    fetch('./api/search?limit=500').then(r => r.json()).then(res => {
      const items = Array.isArray(res) ? res : (res.items || []);
      const srcs = {};
      items.forEach(o => { srcs[o.source || '?'] = (srcs[o.source || '?'] || 0) + 1; });
      const top = Object.entries(srcs).sort((x, y) => y[1] - x[1]).slice(0, 6);
      const maxT = Math.max(...top.map(x => x[1]), 1);
      ...
      if (el && top.length) el.innerHTML = top.map(([t, c], i) => {
        const col = PALETTE[i % PALETTE.length];
        const ic = ['brain', 'sparkles', 'cpu', 'scale', 'shield', 'compass'][i % 6];
        return `<div class="topic-row"><span class="topic-ic" style="background:${col}18;color:${col}">${WOS.icon(ic, 14)}</span><span class="topic-nm">${esc(t)}</span><span class="topic-n">${c}</span><span class="topic-bar"><i style="width:${(c / maxT) * 100}%;background:${col}"></i></span></div>`;
      }).join('');
    }).catch(() => { });
  }
