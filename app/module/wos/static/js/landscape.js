/* ════════════════════════════════════════════════════════════
   WOS Landscape — the vocabulary of the stream.
...
WOS.Landscape = (() => {
  ...
  function afterRender(){
    const C = WOS.Charts;
    const it = items();
    C.onClick('wordCloud', (name)=>WOS.applyFilter({q:name}));
    C.onClick('treemap', (name)=>WOS.applyFilter({q:name}));
    ...
    C.onClick('graph', (name)=>WOS.applyFilter({q:name}));
    ...
    // toggles
    document.querySelectorAll('#landMode .seg-btn').forEach(b=>b.addEventListener('click',()=>{mode=b.dataset.m;WOS.navigate('landscape');}));
    document.querySelectorAll('#landGrams .seg-btn').forEach(b=>b.addEventListener('click',()=>{grams=b.dataset.g;WOS.navigate('landscape');}));
  }
