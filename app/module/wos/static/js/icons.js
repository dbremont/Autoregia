/* ============================================================
   WOS Icons - Self-hosted Lucide icon set (ISC license)
   Curated monoline icons, 1.6px stroke, 24px grid.
   Use:  WOS.icon("gauge")           -> full <svg> markup
         <wos-icon name="gauge"></wos-icon>
   ============================================================ */

;(function (WOS) {
  WOS._icons = {};
  Object.assign(WOS._icons, {
  ...
  WOS.icon = function(name, size) {
    var inner = WOS._icons[name] || WOS._icons["circle"];
    var s = size || 18;
    return "<svg class=\"wos-icon\" width=\"" + s + "\" height=\"" + s + "\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.6\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">" + inner + "</svg>";
  };

  if (!customElements.get("wos-icon")) {
    customElements.define("wos-icon", class extends HTMLElement {
      connectedCallback() {
        var name = this.getAttribute("name") || "circle";
        var size = parseInt(this.getAttribute("size") || "18", 10);
        this.innerHTML = WOS.icon(name, size);
      }
    });
  }
