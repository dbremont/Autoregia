"""WOS analytics — pure aggregation functions over the observation corpus.

Every function here takes a plain list of observation dicts (the shape written
by :mod:`wos.server`) and returns JSON-serialisable structures for the
...
# ── hot now: per-source spike + trending merge ───────────────────────────────
def hot_now(obs: list[dict], now_ms: int, n: int = 12) -> list[dict]:
    """Items per source in the last 24h weighted by 7-day baseline ratio."""
