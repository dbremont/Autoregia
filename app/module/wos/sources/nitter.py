...
tweets. We query a configurable list of instances (env ``WOS_NITTER_INSTANCES``)
and mirror the :mod:`wos.sources.mastodon` pattern: instances come and go, so we
try each one and skip any that fail.

The default instance list is the normalized set of mirrors that serve
``/<handle>/rss`` (2026-09: ``nitter.net``, ``nitter.click``, ``xcancel.com``).
The source is multi-instance by design so a working mirror keeps the sense
organ operational. The instance order may be seeded from the seed file
(``config/seed.json``, ``settings.nitter_instances`` — see
``spec/wos/policy.md``) and is overridden by ``WOS_NITTER_INSTANCES``. The
``native_url`` is rewritten to ``https://twitter.com/<handle>/status/<id>``
so click-throughs land on the original tweet regardless of which mirror
served the feed.
...
appearing in two followed handles into one observation (with merged
tags), exactly like Hacker News item dedup.

``query`` is the handle (``teortexasTex`` or ``@teortexasTex``).
"""
...
_DEFAULT_INSTANCES = "nitter.net,nitter.click,xcancel.com"
