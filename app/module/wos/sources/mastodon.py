...
instances (env ``WOS_MASTODON_INSTANCES``). Most instances allow unauthenticated
reads of the public timeline; set ``WOS_MASTODON_ACCESS_TOKEN`` for instances
that require it.

``query`` is the hashtag (``AI`` or ``#AI``).
"""
...
class MastodonSource:
    ...
    def instances(self) -> list[str]:
        raw = os.environ.get("WOS_MASTODON_INSTANCES", _DEFAULT_INSTANCES)
        return [i.strip() for i in raw.split(",") if i.strip()]

    @property
    def _token(self) -> str:
        return os.environ.get("WOS_MASTODON_ACCESS_TOKEN", "")
