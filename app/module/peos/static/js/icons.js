/* ============================================================
   PEOS Icons - Self-hosted Lucide icon set (ISC license)
   Curated monoline icons, 1.6px stroke, 24px grid.
   Use:  PEOS.icon("gauge")           -> full <svg> markup
         <peos-icon name="gauge"></peos-icon>
   ============================================================ */

;(function (PEOS) {
  PEOS._icons = {};
  Object.assign(PEOS._icons, {
  ...
  PEOS.icon = function(name, size) {
    var inner = PEOS._icons[name] || PEOS._icons["circle"];
    var s = size || 18;
    return "<svg class=\"peos-icon\" width=\"" + s + "\" height=\"" + s + "\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.6\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\">" + inner + "</svg>";
  };

  if (!customElements.get("peos-icon")) {
    customElements.define("peos-icon", class extends HTMLElement {
      connectedCallback() {
        var name = this.getAttribute("name") || "circle";
        var size = parseInt(this.getAttribute("size") || "18", 10);
        this.innerHTML = PEOS.icon(name, size);
      }
    });
  }
